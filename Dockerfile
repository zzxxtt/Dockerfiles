...
RUN \
    ...
    cmake -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_DOCS=NO -DBUILD_EXAMPLES=NO -DOPENCV_EXTRA_MODULES_PATH=/root/opencv_contrib-3.3.0/modules -DWITH_CUDA=NO -DWITH_OPENMP=YES -DWITH_CUBLAS=NO -DBUILD_PYTHON_SUPPORT=ON -DBUILD_opencv_python2=ON -DBUILD_opencv_python3=ON -DPYTHON3_EXECUTABLE=$(which python3) -DPYTHON3_INCLUDE_DIRS=$(python3 -c "from distutils.sysconfig import get_python_inc; print(get_python_inc())") -DPYTHON3_PACKAGES_PATH=$(python3 -c "from distutils.sysconfig import get_python_lib; print(get_python_lib())") -DPYTHON3_LIBRARY=/usr/lib/x86_64-linux-gnu/libpython3.5m.so -DWITH_1394=OFF .. && \
